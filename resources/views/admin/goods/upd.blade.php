		...
		<div class="page ">
			...
			<div class="banneradd bor">
				...
				<div class="baBody">
					@csrf
					<div class="bbD">
						商品名称：<input type="text" name="goods_name" class="input3" value="{{$data->goods_name}}" />
					</div>
					<div class="bbD">
						商品数量：<input type="text" name="goods_number" class="input3" value="{{$data->goods_number}}" />
					</div>
					<div class="bbD">
						商品价格：<input type="text" name="goods_price" class="input3" value="{{$data->goods_price}}" />
					</div>
					<div class="bbD">
						商品货号：<input type="text" name="goods_sn" class="input3" value="{{$data->goods_sn}}" />
					</div>
					<div class="bbD">
                    <input type="hidden" name="oldimg" value='{{$data->goods_img}}'>
						商品图片：<input type="file" name="goods_img"  />
                        <img src="{{env('UPLOAD_URL')}}{{$data->goods_img}}" height="100" width="100">
					</div>
					<div class="bbD">
                    ...
                    <div class="bbD">
						商品详情：<textarea class="input3" name="goods_desc" cols="30" rows="10">{{$data->goods_desc}}</textarea>
					</div>
					<div class="bbD">
					...
					<div class="bbD">
						是否热卖：<label><input type="radio"  name="is_hot" value="1" @if($data->is_hot==1) checked="checked" @endif />&nbsp;是</label><label><input type="radio" name="is_hot" value="0" @if($data->is_hot==0) checked="checked" @endif />&nbsp;否</label>
					</div>
					<div class="bbD">
						是否新品：<label><input type="radio" name="is_new" value="1" @if($data->is_new==1) checked="checked" @endif/>&nbsp;是</label><label><input type="radio" name="is_new" value="0" @if($data->is_new==0) checked="checked" @endif/>&nbsp;否</label>
					</div>
					<div class="bbD">
						是否上架：<label><input type="radio" name="is_on_sale" value="1" @if($data->is_on_sale==1) checked="checked" @endif/>&nbsp;是</label><label><input type="radio" name="is_on_sale" value="0" @if($data->is_on_sale==0) checked="checked" @endif/>&nbsp;否</label>
					</div>
					<div class="bbD">
						<p class="bbDP">
